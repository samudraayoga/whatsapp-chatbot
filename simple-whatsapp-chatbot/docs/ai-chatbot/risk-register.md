...
|---|---|---:|---:|---|---|---|---|
| R-001 | Knowledge not ready or incomplete | High | High | Sprint 2 governance tooling and 12-category seed; assign owner/category/source/validity; weekly review; unanswered workflow | Low coverage, repeated fallback, missing approved source | Knowledge Owner | Readiness gate implemented; real content/owner blocked |
...
| R-004 | Retrieval misses correct knowledge | High | High | Chunk review, variants, metadata, Recall@K, evaluated threshold, re-index/version metrics | Low Recall@K, low similarity despite known answer | AI Lead | Internal search and formal metric gate implemented; real evaluation dataset pending |
...
| R-016 | Stale knowledge remains in cache/index | Medium | High | Version-aware cache key, publish/archive invalidation, index generation, active/valid filters | Archived/expired source appears in answer | Backend + AI Lead | Version-aware runtime cache and archive filters implemented/live-smoked |
| R-017 | Feature appears enabled before dependencies/approval are ready | Medium | Critical | Startup hard-off, fail-closed readiness, auditable evidence, staged pilot request, explicit effective 0%, and emergency pause | Foundation reports enabled while blocker exists, or a customer provider call occurs | Product + Tech Lead | Sprint 8 tenant pilot controls implemented; customer adapter remains hard-off |
...
| R-020 | Observability misses unsafe or costly behavior | Medium | High | Trace/model/source/token/latency, safe metrics, alert, review dashboard, no-PII logs | Untraceable answer or delayed provider/cost incident | SRE + QA | Dashboard, daily review, and threshold settings implemented; production alert routing owner blocked |
| R-021 | Evaluation dataset is biased or too small | High | High | 100–300 mixed/adversarial cases, real-language patterns, Medical/Product review, versioning | Pilot errors not represented in dataset | QA + Medical | Import/report gate and starter template implemented; approved real dataset/owner blocked |
...
| R-024 | Vulnerable production dependency enters release | Medium | High | Audit every package/submodule, patch SLA, lockfile review, secret/container scan, time-boxed exception | High/Critical audit finding | Security + Maintainer | Prior anti-ban production finding remediated and regression-tested; continuous scanning still required |
...
## Resolved concrete finding

At the Sprint 3 verification date, production dependency audit reported one High
finding in `baileys-antiban`:
...
Sprint 8 added an override to patched `ip-address@10.4.x`, updated the nested
lockfile, and passed anti-ban typecheck, manual regression, ESM/CJS build, and
production dependency audit. Backend, frontend, and anti-ban production audits
now report zero vulnerabilities at verification time. Auditing only the parent
backend lockfile remains insufficient because the local package has its own lockfile.
